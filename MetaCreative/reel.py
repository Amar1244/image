...
Added: Emotion system — ad phase detection + topic-matched emotion expressions
"""
...
# ─────────────────────────────────────────────
# ✅ EMOTION SYSTEM (added from Code 1)
# ─────────────────────────────────────────────

_RESULTS_KEYWORDS = [
    "result", "results", "before after", "before & after", "transformation",
    "success", "testimonial", "review", "worked", "it worked", "solved",
    "cured", "regrew", "regrowth achieved", "happy customer", "case study"
]
_SOLUTION_KEYWORDS = [
    "treatment", "solution", "therapy", "product", "medicine", "remedy",
    "doctor", "clinic", "consult", "book", "appointment", "advise",
    "dermatologist", "trichologist", "ayurvedic", "homeopathic",
    "recommend", "plan", "programme", "program", "kit", "serum", "oil",
    "shampoo", "tablet", "capsule", "supplement"
]
_PROBLEM_KEYWORDS = [
    "problem", "issue", "concern", "worried", "stress", "anxiety",
    "suffering", "struggling", "losing", "falling", "thinning", "receding",
    "bald", "dandruff", "pcos", "thyroid", "postpartum", "acne", "dark spots",
    "oily", "dry skin", "wrinkle", "ageing", "aging", "dark circle",
    "hair loss", "hair fall", "hairfall", "weight", "obesity", "pain"
]


def detect_ad_phase(topic: str) -> str:
    t = topic.lower()
    if any(kw in t for kw in _RESULTS_KEYWORDS):
        return "results"
    if any(kw in t for kw in _SOLUTION_KEYWORDS):
        return "solution"
    if any(kw in t for kw in _PROBLEM_KEYWORDS):
        return "problem"
    return "problem"
...
def _detect_topic_category(topic: str) -> str:
    t = topic.lower()
    _hair_category_kws = HAIR_LOSS_KEYWORDS + ["hair", "hairline", "follicle", "regrow"]
    if any(kw in t for kw in _hair_category_kws):
        return "hair"
    if any(kw in t for kw in ["skin", "acne", "dark spot", "wrinkle", "glow", "fairness", "pigment"]):
        return "skin"
    if any(kw in t for kw in ["pcos", "thyroid", "hormonal", "menstrual", "period"]):
        return "hormonal"
    if any(kw in t for kw in ["weight", "obesity", "fat", "slim", "diet"]):
        return "weight"
    if any(kw in t for kw in ["stress", "anxiety", "sleep", "insomnia", "mental"]):
        return "stress"
    if any(kw in t for kw in ["pain", "joint", "back", "knee", "arthritis"]):
        return "pain"
    return "general"
...
_EMOTION_MAP = {
    ("problem", "skin"): (
        "Self-conscious and insecure — touching face lightly, avoiding eye contact or "
        "looking sideways. Slight frown, tense jaw. Quiet shame. NOT smiling."
    ),
    ("solution", "skin"): (
        "Curious and hopeful — leaning slightly forward, head tilted, soft expectant smile. "
        "Eyes attentive, not fully relaxed but optimistic."
    ),
    ("results", "skin"): (
        "Glowing, open, radiant smile — direct eye contact, relaxed jaw, chin slightly lifted. "
        "Confident and at ease. Joyful without being over-the-top."
    ),
    ("problem", "hormonal"): (
        "Quietly overwhelmed — tired eyes, slight downward gaze, lips pressed together. "
        "Carrying invisible weight. NOT smiling."
    ),
    ("solution", "hormonal"): (
        "Measured relief — soft exhale visible in posture, slight uplift at corners of mouth. "
        "Eyes calmer, shoulders starting to drop."
    ),
    ("results", "hormonal"): (
        "Warm, composed confidence — gentle genuine smile, steady direct gaze. "
        "Energised but not hyper."
    ),
    ("problem", "weight"): (
        "Frustrated and discouraged — arms crossed, slight slump, looking slightly down. "
        "Quiet despair. NOT smiling."
    ),
    ("solution", "weight"): (
        "Determined and motivated — chin slightly lifted, slight firm smile. "
        "Eyes focused. Standing taller."
    ),
    ("results", "weight"): (
        "Proud and energised — big authentic smile, open posture, hands relaxed. "
        "Glowing vitality."
    ),
    ("problem", "stress"): (
        "Visibly exhausted and overwhelmed — heavy eyelids, tense neck, slumped shoulders. "
        "Quiet desperation. NOT smiling."
    ),
    ("solution", "stress"): (
        "Gentle relief beginning — slower breath visible, eyes softer, jaw unclenching. "
        "Small tentative smile."
    ),
    ("results", "stress"): (
        "Serene and rested — relaxed open face, soft genuine smile, calm bright eyes. "
        "Unhurried, grounded posture."
    ),
    ("problem", "pain"): (
        "Tense and guarded — slight wincing around eyes, one hand near the area of pain. "
        "Mouth corners pulled down. NOT smiling."
    ),
    ("solution", "pain"): (
        "Cautiously optimistic — slightly less tense jaw, head tilted with interest. "
        "One eyebrow raised, hope in the eyes."
    ),
    ("results", "pain"): (
        "Free and mobile — natural wide smile, open relaxed posture. "
        "Moving with ease."
    ),
    ("problem", "general"): (
        "Concerned and thoughtful — slight frown, head tilted, gaze directed inward. "
        "Quiet worry without panic."
    ),
    ("solution", "general"): (
        "Engaged and trusting — slight forward lean, attentive open eyes, small smile."
    ),
    ("results", "general"): (
        "Relaxed confident happiness — natural smile, open shoulders, direct eye contact. "
        "Genuine warmth."
    ),
}


def get_emotion_instruction(topic: str, gender: str) -> tuple:
    """Returns (emotion_instruction, phase_label, emotion_short)"""
    phase    = detect_ad_phase(topic)
    category = _detect_topic_category(topic)
    key      = (phase, category)

    # HAIR OVERRIDE — always worried/sad
    if category == "hair":
        instruction = (
            "Deeply worried and visibly sad — furrowed brow, heavy tired eyes with dark circles, "
            "downward gaze or staring into distance. Jaw slightly tense, lips pressed together or slightly parted. "
            "Quiet emotional pain. NOT smiling. NOT confident. Expression of someone silently struggling "
            "with hair loss — sad, concerned, and emotionally affected."
        )
        if gender == "female":
            instruction += " Subtle feminine vulnerability — soft eyes glistening, personal and private moment."
        else:
            instruction += " Subtle masculine quiet — holding it in, not showing it openly but clearly affected."
        return instruction, phase.capitalize(), "😢 Worried & Sad"

    instruction = _EMOTION_MAP.get(key, _EMOTION_MAP[("problem", "general")])
    if gender == "female":
    ...
    emotion_short = {
        ("problem", "skin"):     "😔 Insecure & Self-Conscious",
        ("solution", "skin"):    "🙂 Cautiously Hopeful",
        ("results", "skin"):     "✨ Radiant Confidence",
        ("problem", "hormonal"): "😶 Quietly Overwhelmed",
        ("solution", "hormonal"):"😌 Measured Relief",
        ("results", "hormonal"): "🌸 Balanced & Composed",
        ("problem", "weight"):   "😞 Discouraged",
        ("solution", "weight"):  "💪 Determined",
        ("results", "weight"):   "🎉 Proud & Energised",
        ("problem", "stress"):   "😩 Exhausted & Overwhelmed",
        ("solution", "stress"):  "😮‍💨 Stress Lifting",
        ("results", "stress"):   "😴 Serene & Rested",
        ("problem", "pain"):     "😣 Tense & Guarded",
        ("solution", "pain"):    "🤔 Cautiously Optimistic",
        ("results", "pain"):     "🙌 Free & Mobile",
        ("problem", "general"):  "😟 Concerned",
        ("solution", "general"): "🙂 Engaged & Trusting",
        ("results", "general"):  "😊 Confident & Relieved",
    }.get(key, "🙂 Natural Expression")

    return instruction, phase.capitalize(), emotion_short
...
# ✅ EMOTION injected into build_general_subject
def build_general_subject(gender, topic=""):
    emotion_instruction, phase_label, emotion_short = get_emotion_instruction(topic, gender) if topic else (
        "Natural confident expression.", "Problem", "🙂 Natural"
    )
    if gender == "male":
        ...
        subject = (
            ...
            f"EMOTION & EXPRESSION: {emotion_instruction} "
        ...
        return subject, "Standard", light["label"], pose["label"], phase_label, emotion_short
    else:
...
# ✅ EMOTION injected into build_hair_subject
def build_hair_subject(gender, topic=""):
    ...
    if gender == "male":
        ...
        return subject, hair["label"], light["label"], pose["label"], phase_label, emotion_short
    else:
...
def _build_base_prompt(style, title, subtext, cta, font, layout, hair_gender=None, is_hair_topic=False, topic=""):
    hair_label = lighting_label = pose_label = phase_label = emotion_short = None
    if hair_gender and is_hair_topic:
        photography, hair_label, lighting_label, pose_label, phase_label, emotion_short = build_hair_subject(hair_gender, topic=topic)
    elif hair_gender:
        photography, hair_label, lighting_label, pose_label, phase_label, emotion_short = build_general_subject(hair_gender, topic=topic)
    else:
        photography = (
    ...
    return prompt, hair_label, lighting_label, pose_label, phase_label, emotion_short
...
for key, default in {
    ...
    "ad_phase": None, "emotion_short": None,  # ✅ new emotion state keys
}.items():
...
with left_col:
    ...
    if topic:
        ...
        _live_category = _detect_topic_category(topic)
    ...
    if st.button("✦ Generate Ideas", disabled=not topic):
        if not st.session_state.get("groq_key"):
            ...
            st.session_state["emotion_short"]   = None  # ✅
    ...
    if st.session_state["ad_data"]:
        ...
        def build_and_store(style, title, subtext, cta, font, layout):
            ...
            built, hl, ll, pl, phase_lbl, emo_short = _build_base_prompt(
            ...
            st.session_state["emotion_short"]  = emo_short   # ✅
        ...
        if st.session_state["skeleton_square"] and st.session_state["skeleton_story"]:
            ...
            # ✅ Character details — now includes emotion + phase
            ...
            em_p  = st.session_state.get("emotion_short")
            if hg_p and hl_p:
        ...
        if st.session_state["base_prompt"]:
            ...
            em   = st.session_state.get("emotion_short", "")
            ...
            if hg and hl:
                ...
                st.markdown(
                    ...
                    f"✅ {gw} · Hair: <b>{hl}</b> · Lighting: <b>{ll}</b>{pose_text}{phase_text}{emotion_text} · 🎞 Raw Emotional</span>",
                    unsafe_allow_html=True
                )
