...
# ─────────────────────────────────────────────
# EMOTION SYSTEM (from PDF documentation)
# ─────────────────────────────────────────────
...
def detect_ad_phase(topic):
    """Detect if topic is Problem, Solution, or Results phase"""
...
def detect_topic_category(topic):
    """Detect which category the topic belongs to"""
    topic_lower = topic.lower()
    ...
    if any(kw in topic_lower for kw in HAIR_CAT):
        return "hair"
    elif any(kw in topic_lower for kw in SKIN_CAT):
        return "skin"
    elif any(kw in topic_lower for kw in HORMONAL_CAT):
        return "hormonal"
    elif any(kw in topic_lower for kw in WEIGHT_CAT):
        return "weight"
    elif any(kw in topic_lower for kw in STRESS_CAT):
        return "stress"
    elif any(kw in topic_lower for kw in PAIN_CAT):
        return "pain"
    return "general"

def get_emotion_instruction(topic, gender):
    """Returns appropriate emotion instruction based on topic phase and category"""
    phase = detect_ad_phase(topic)
    category = detect_topic_category(topic)
    
    # HAIR always gets worried/sad (per PDF page 4)
    if category == "hair":
        instruction = (
...
# ─────────────────────────────────────────────
# SUBJECT BUILDERS (with emotion system)
# ─────────────────────────────────────────────
def build_general_subject(gender, topic=""):
    emotion_instruction, phase, emotion_emoji = get_emotion_instruction(topic, gender) if topic else ("Natural confident expression.", "problem", "🙂 Natural")
...
def build_hair_subject(gender, topic=""):
    emotion_instruction, phase, emotion_emoji = get_emotion_instruction(topic, gender) if topic else (
...
def _build_base_prompt(style, title, subtext, cta, font, layout, hair_gender=None, is_hair_topic=False, topic=""):
    hair_label = lighting_label = pose_label = phase_label = emotion_emoji = None
    
    if hair_gender and is_hair_topic:
        photography, hair_label, lighting_label, pose_label, phase_label, emotion_emoji = build_hair_subject(hair_gender, topic=topic)
    elif hair_gender:
        photography, hair_label, lighting_label, pose_label, phase_label, emotion_emoji = build_general_subject(hair_gender, topic=topic)
    else:
        photography = (
...
with left_col:
    ...
    topic = st.text_input("Topic", placeholder="e.g. acne treatment, hair regrowth, PCOS solution, stress relief...",
    ...
    if st.session_state["ad_data"]:
        ...
        if st.session_state["skeleton_square"] and st.session_state["skeleton_story"]:
            ...
            # Character details with emotion
