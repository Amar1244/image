"""
Meta Creative Generator - COMPLETE VERSION
...
Fix 3: EMOTION SYSTEM — Problem/Solution/Results detection for all topics
Fix 4: GENDER DETECTION — LLM decides gender based on medical condition (PCOS→female, prostate→male)
ADDED: Emotion override dropdown + Prompt editor
"""
...
# ─────────────────────────────────────────────
PROBLEM_KEYWORDS = [
    "hair loss", "hair fall", "acne", "pimple", "breakout", "dandruff",
    "thinning", "receding", "bald", "alopecia", "concern", "worry",
    "stress", "anxiety", "pain", "suffering", "struggling", "problem"
]

SOLUTION_KEYWORDS = [
    "treatment", "cure", "remedy", "therapy", "medicine", "solution",
    "doctor", "clinic", "consult", "appointment", "help", "fix", "solve"
]

RESULTS_KEYWORDS = [
    "result", "success", "before after", "transformation", "worked",
    "improved", "better", "regrew", "healed", "clear", "glowing", "before & after"
]

def detect_ad_phase(topic):
    topic_lower = topic.lower()
    if any(kw in topic_lower for kw in RESULTS_KEYWORDS):
        return "results"
    elif any(kw in topic_lower for kw in SOLUTION_KEYWORDS):
        return "solution"
    else:
        return "problem"
...
def detect_topic_category(topic):
    topic_lower = topic.lower()
    HAIR_CAT = ["hair", "bald", "alopecia", "dandruff", "scalp", "hairline"]
    SKIN_CAT = ["acne", "pimple", "breakout", "skin", "dark spot", "pigmentation", "scar", "oily", "dry skin"]
    HORMONAL_CAT = ["pcos", "thyroid", "hormonal", "period", "menstrual", "pcod"]
    WEIGHT_CAT = ["weight", "obesity", "fat", "slim", "belly"]
    STRESS_CAT = ["stress", "anxiety", "tension", "pressure", "deadline", "burnout"]
    PAIN_CAT = ["pain", "joint", "back", "knee", "arthritis", "ache", "headache"]
    if any(kw in topic_lower for kw in HAIR_CAT): return "hair"
    elif any(kw in topic_lower for kw in SKIN_CAT): return "skin"
    elif any(kw in topic_lower for kw in HORMONAL_CAT): return "hormonal"
    elif any(kw in topic_lower for kw in WEIGHT_CAT): return "weight"
    elif any(kw in topic_lower for kw in STRESS_CAT): return "stress"
    elif any(kw in topic_lower for kw in PAIN_CAT): return "pain"
    return "general"

MANUAL_EMOTION_MAP = {
    "Auto (default)": None,
    "😢 Worried & Sad - Deep distress": (
        "EMOTION: Deeply worried and visibly sad — furrowed brow, heavy tired eyes with dark circles, "
        "downward gaze or staring into distance. NOT smiling. Quiet emotional pain.",
        "problem", "😢 Worried & Sad"
    ),
    "😔 Insecure - Self-conscious, avoiding eye contact": (
        "EMOTION: Self-conscious and insecure — touching face lightly, avoiding direct eye contact, "
        "looking slightly sideways. Slight frown, tense jaw, quiet shame. NOT smiling.",
        "problem", "😔 Insecure"
    ),
    "🙂 Cautiously Hopeful - Small smile, expectant": (
        "EMOTION: Cautiously hopeful — leaning slightly forward, head tilted, soft expectant smile. "
        "Eyes attentive, not fully relaxed but optimistic.",
        "solution", "🙂 Cautiously Hopeful"
    ),
    "✨ Radiant Confidence - Glowing, joyful": (
        "EMOTION: Glowing, open, radiant smile — direct eye contact, relaxed jaw, chin slightly lifted. "
        "Confident and at ease. Joyful without being over-the-top.",
        "results", "✨ Radiant Confidence"
    ),
    "😶 Overwhelmed - Quiet burden": (
        "EMOTION: Quietly overwhelmed — tired eyes, slight downward gaze, lips pressed together. Carrying invisible weight. NOT smiling.",
        "problem", "😶 Overwhelmed"
    ),
    "😌 Measured Relief - Calm, peaceful": (
        "EMOTION: Measured relief — soft exhale visible in posture, slight uplift at corners of mouth. Eyes calmer.",
        "solution", "😌 Measured Relief"
    ),
    "😞 Discouraged - Frustrated, arms crossed": (
        "EMOTION: Frustrated and discouraged — arms crossed, slight slump, looking slightly down. Quiet despair. NOT smiling.",
        "problem", "😞 Discouraged"
    ),
    "💪 Determined - Motivated, focused": (
        "EMOTION: Determined and motivated — chin slightly lifted, slight firm smile. Eyes focused. Standing taller.",
        "solution", "💪 Determined"
    ),
    "🎉 Proud & Energised - Big smile, open posture": (
        "EMOTION: Proud and energised — big authentic smile, open posture, hands relaxed. Glowing vitality.",
        "results", "🎉 Proud & Energised"
    ),
    "😩 Exhausted - Heavy eyes, slumped": (
        "EMOTION: Visibly exhausted and overwhelmed — heavy eyelids, tense neck, slumped shoulders. Quiet desperation. NOT smiling.",
        "problem", "😩 Exhausted"
    ),
    "😣 In Pain - Tense, wincing": (
        "EMOTION: Tense and guarded — slight wincing around eyes, one hand near the area of pain. Mouth corners pulled down. NOT smiling.",
        "problem", "😣 In Pain"
    ),
    "🙌 Pain Free - Free, mobile, smiling": (
        "EMOTION: Free and mobile — natural wide smile, open relaxed posture. Moving with ease.",
        "results", "🙌 Pain Free"
    ),
    "😊 Natural - Relaxed, authentic": (
        "EMOTION: Natural, authentic, relatable expression appropriate to the context.",
        "general", "😊 Natural"
    ),
}

def get_emotion_instruction(topic, gender, manual_emotion=None, use_auto=True):
    if not use_auto and manual_emotion and manual_emotion in MANUAL_EMOTION_MAP and MANUAL_EMOTION_MAP[manual_emotion] is not None:
        instruction, phase, emoji = MANUAL_EMOTION_MAP[manual_emotion]
        if gender == "female":
            instruction += " Subtle feminine vulnerability or strength as appropriate."
        else:
            instruction += " Subtle masculine stoicism or quiet pride as appropriate."
        return instruction, phase, emoji
    ...
    if category == "hair":
        instruction = (
            "EMOTION: Deeply worried and visibly sad — furrowed brow, heavy tired eyes with dark circles, "
            "downward gaze or staring into distance. Jaw slightly tense, lips pressed together. "
            "Quiet emotional pain. NOT smiling. NOT confident."
        )
        if gender == "female":
            instruction += " Subtle feminine vulnerability — soft eyes glistening, personal private moment."
        else:
            instruction += " Subtle masculine quiet — holding it in, not showing it openly but clearly affected."
        return instruction, phase, "😢 Worried & Sad"

    if category == "skin" and phase == "problem":
        instruction = (
            "EMOTION: Self-conscious and insecure — touching face lightly, avoiding direct eye contact, "
            "looking slightly sideways. Slight frown, tense jaw, quiet shame. NOT smiling."
        )
        emoji = "😔 Insecure"
    elif category == "skin" and phase == "solution":
        instruction = (
            "EMOTION: Cautiously hopeful — leaning slightly forward, head tilted, soft expectant smile. "
            "Eyes attentive, not fully relaxed but optimistic."
        )
        emoji = "🙂 Cautiously Hopeful"
    elif category == "skin" and phase == "results":
        instruction = (
            "EMOTION: Glowing, open, radiant smile — direct eye contact, relaxed jaw, chin slightly lifted. "
            "Confident and at ease. Joyful without being over-the-top."
        )
        emoji = "✨ Radiant Confidence"
    elif category == "hormonal" and phase == "problem":
        instruction = "EMOTION: Quietly overwhelmed — tired eyes, slight downward gaze, lips pressed together. Carrying invisible weight. NOT smiling."
        emoji = "😶 Overwhelmed"
    elif category == "hormonal" and phase == "solution":
        instruction = "EMOTION: Measured relief — soft exhale visible in posture, slight uplift at corners of mouth. Eyes calmer."
        emoji = "😌 Measured Relief"
    elif category == "hormonal" and phase == "results":
        instruction = "EMOTION: Warm, composed confidence — gentle genuine smile, steady direct gaze. Energised but not hyper."
        emoji = "🌸 Balanced"
    elif category == "weight" and phase == "problem":
        instruction = "EMOTION: Frustrated and discouraged — arms crossed, slight slump, looking slightly down. Quiet despair. NOT smiling."
        emoji = "😞 Discouraged"
    elif category == "weight" and phase == "solution":
        instruction = "EMOTION: Determined and motivated — chin slightly lifted, slight firm smile. Eyes focused. Standing taller."
        emoji = "💪 Determined"
    elif category == "weight" and phase == "results":
        instruction = "EMOTION: Proud and energised — big authentic smile, open posture, hands relaxed. Glowing vitality."
        emoji = "🎉 Proud & Energised"
    elif category == "stress" and phase == "problem":
        instruction = "EMOTION: Visibly exhausted and overwhelmed — heavy eyelids, tense neck, slumped shoulders. Quiet desperation. NOT smiling."
        emoji = "😩 Exhausted"
    elif category == "stress" and phase == "solution":
        instruction = "EMOTION: Gentle relief beginning — slower breath visible, eyes softer, jaw unclenching. Small tentative smile."
        emoji = "😮‍💨 Stress Lifting"
    elif category == "stress" and phase == "results":
        instruction = "EMOTION: Serene and rested — relaxed open face, soft genuine smile, calm bright eyes. Unhurried, grounded posture."
        emoji = "😌 Serene & Rested"
    elif category == "pain" and phase == "problem":
        instruction = "EMOTION: Tense and guarded — slight wincing around eyes, one hand near the area of pain. Mouth corners pulled down. NOT smiling."
        emoji = "😣 In Pain"
    elif category == "pain" and phase == "solution":
        instruction = "EMOTION: Cautiously optimistic — slightly less tense jaw, head tilted with interest. One eyebrow raised, hope in the eyes."
        emoji = "🤔 Cautiously Optimistic"
    elif category == "pain" and phase == "results":
        instruction = "EMOTION: Free and mobile — natural wide smile, open relaxed posture. Moving with ease."
        emoji = "🙌 Pain Free"
    else:
        instruction = "EMOTION: Natural, authentic, relatable expression appropriate to the context."
        emoji = "🙂 Natural"

    if gender == "female":
        instruction += " Subtle feminine vulnerability or strength as appropriate to the phase."
    else:
        instruction += " Subtle masculine stoicism or quiet pride as appropriate to the phase."
...
# SUBJECT BUILDERS
# ─────────────────────────────────────────────
def build_general_subject(gender, topic="", manual_emotion=None, use_auto=True):
    emotion_instruction, phase, emotion_emoji = get_emotion_instruction(
        topic, gender, manual_emotion, use_auto
    ) if topic else ("Natural confident expression.", "problem", "🙂 Natural")

    if gender == "male":
        pose = random.choice(GENERAL_MALE_POSES)
        light = random.choice(GENERAL_LIGHTING)
        bg_override, bg_label = detect_background(topic)
        final_bg = bg_override if bg_override else light["bg"]
        subject = (
            ...
            f"LIGHTING: {light['desc']} BACKGROUND: {final_bg}."
            "Photorealistic, natural Indian skin tone, ultra detailed 8K. "
            "Single person only, no products."
        )
        return subject, "Standard", light["label"], pose["label"], phase, emotion_emoji
    else:
...
def build_hair_subject(gender, topic="", manual_emotion=None, use_auto=True):
    emotion_instruction, phase, emotion_emoji = get_emotion_instruction(
        topic, gender, manual_emotion, use_auto
    ) if topic else (
        "Deeply worried and visibly sad — furrowed brow, heavy tired eyes. NOT smiling.",
        "problem", "😢 Worried & Sad"
    )
    ...
    if gender == "male":
        ...
        return subject, hair["label"], light["label"], pose["label"], phase, emotion_emoji
    else:
...
# ─────────────────────────────────────────────
# GROQ API — WITH LLM GENDER DETECTION
# ─────────────────────────────────────────────
def call_groq(api_key, topic):
    ...
    system_prompt = (
        ...
        "gender (MUST be either 'male' or 'female' based on the medical condition. "
        "Examples: PCOS, pregnancy, menopause, breast cancer → 'female'. "
        "Prostate, testicular issues, erectile dysfunction → 'male'. "
        "Hair fall, skin issues, stress, weight → randomly pick 'male' or 'female'). "
...
def _build_base_prompt(style, title, subtext, cta, font, layout, hair_gender=None, is_hair_topic=False, topic="", manual_emotion=None, use_auto=True):
    hair_label = lighting_label = pose_label = phase_label = emotion_emoji = None

    if hair_gender and is_hair_topic:
        photography, hair_label, lighting_label, pose_label, phase_label, emotion_emoji = build_hair_subject(
            hair_gender, topic=topic, manual_emotion=manual_emotion, use_auto=use_auto
        )
    elif hair_gender:
        photography, hair_label, lighting_label, pose_label, phase_label, emotion_emoji = build_general_subject(
            hair_gender, topic=topic, manual_emotion=manual_emotion, use_auto=use_auto
        )
    else:
        photography = (
    ...
    return prompt, hair_label, lighting_label, pose_label, phase_label, emotion_emoji
...
for key, default in {
    ...
    "ad_phase": None, "emotion_emoji": None,
    "use_auto_emotion": True, "manual_emotion_select": "Auto (default)",
    "gender_source": "groq",
}.items():
...
st.markdown("<p style='text-align:center;color:#888;font-size:13px;margin-bottom:20px'>Generates <b>1080×1080</b> Feed + <b>1080×1920</b> Story · Same seed · Proper clothing · Full body visible · LLM detects gender</p>", unsafe_allow_html=True)
...
with left_col:
    ...
    topic = st.text_input("Topic", placeholder="e.g. PCOS treatment, prostate health, acne solution, hair regrowth...",
    ...
    if topic:
        _live_phase = detect_ad_phase(topic)
        _live_category = detect_topic_category(topic)
        _phase_colors = {"problem": "#c0392b", "solution": "#1a73e8", "results": "#27ae60"}
        _phase_icons = {"problem": "😟", "solution": "🤞", "results": "🎉"}
        _pc = _phase_colors.get(_live_phase, "#666")
        _pi = _phase_icons.get(_live_phase, "🙂")
        st.markdown(
            f"<div style='background:{_pc}12;border:1px solid {_pc}40;border-radius:8px;"
            f"padding:6px 12px;font-size:12px;color:{_pc};margin-bottom:6px'>"
            f"{_pi} Ad Phase detected: <b>{_live_phase.capitalize()}</b> &nbsp;·&nbsp; "
            f"Category: <b>{_live_category.capitalize()}</b></div>",
            unsafe_allow_html=True
        )
    ...
    # ── Emotion Control ──────────────────────
    st.markdown("---")
    st.markdown("##### 🎭 Emotion Control")

    col_auto, col_manual = st.columns(2)
    with col_auto:
        use_auto_emotion = st.checkbox("🤖 Auto-detect emotion", value=st.session_state.get("use_auto_emotion", True),
                                        help="Let AI detect emotion based on topic phase and category")
    with col_manual:
        manual_emotion_options = list(MANUAL_EMOTION_MAP.keys())
        manual_emotion = st.selectbox(
            "🎨 Manual emotion override",
            options=manual_emotion_options,
            index=manual_emotion_options.index(st.session_state.get("manual_emotion_select", "Auto (default)")),
            help="Override auto-detected emotion with a specific expression"
        )

    st.session_state["use_auto_emotion"] = use_auto_emotion
    st.session_state["manual_emotion_select"] = manual_emotion
    ...
    if st.session_state["ideas_generated"] and st.session_state["hair_gender"]:
        ...
        gender_note = "🧠 LLM-detected gender based on medical condition" if st.session_state.get("gender_source") == "groq" else "🎲 Random 50/50 fallback"
        st.markdown(
            ...
            f"<span style='opacity:0.65'>({gender_note})</span>"
    ...
    if st.button("✦ Generate Ideas", disabled=not topic):
        if not st.session_state.get("groq_key"):
            ...
            st.session_state["hair_gender"] = None
            ...
            st.session_state["ad_phase"] = None
            st.session_state["emotion_emoji"] = None
            with st.spinner("Groq is writing ad copy and detecting gender..."):
                try:
                    data = call_groq(st.session_state["groq_key"], topic)

                    groq_gender = data.get("gender", None)
                    if groq_gender and groq_gender in ["male", "female"]:
                        st.session_state["hair_gender"] = groq_gender
                        st.session_state["gender_source"] = "groq"
                    else:
                        st.session_state["hair_gender"] = "male" if random.random() < 0.50 else "female"
                        st.session_state["gender_source"] = "fallback"
    ...
    if st.session_state["ad_data"]:
        ...
        def build_and_store(style, title, subtext, cta, font, layout):
            ...
            use_auto  = st.session_state.get("use_auto_emotion", True)
            manual_emo = st.session_state.get("manual_emotion_select", "Auto (default)")
            if manual_emo == "Auto (default)":
                manual_emo = None

            built, hl, ll, pl, phase_lbl, emo = _build_base_prompt(
            ...
            st.session_state["ad_phase"]         = phase_lbl
            st.session_state["emotion_emoji"]    = emo
        ...
        if regenerate_prompt_clicked:
            ...
            with st.spinner("Getting new visual style from Groq..."):
                try:
                    ...
                    new_gender = new_data.get("gender", None)
                    if new_gender and new_gender in ["male", "female"]:
                        st.session_state["hair_gender"]   = new_gender
                        st.session_state["gender_source"] = "groq"
                    if new_style:
        ...
        if st.session_state["skeleton_square"] and st.session_state["skeleton_story"]:
            ...
            ph_p = st.session_state.get("ad_phase")
            em_p = st.session_state.get("emotion_emoji")
            gs_p = st.session_state.get("gender_source", "")
            if hg_p and hl_p:
                gender_icon  = "👨" if hg_p == "male" else "👩"
                gender_word  = "Male" if hg_p == "male" else "Female"
                source_text  = "(LLM-detected)" if gs_p == "groq" else "(50/50 fallback)"
                emotion_row  = f"&nbsp;&nbsp;🎭 <b>Emotion:</b> {em_p}<br>" if em_p else ""
                phase_row    = f"&nbsp;&nbsp;📢 <b>Ad Phase:</b> {ph_p}<br>" if ph_p else ""
                st.markdown(
                    ...
                    f"&nbsp;&nbsp;{gender_icon} <b>Gender:</b> {gender_word} {source_text}<br>"
        ...
        if st.session_state["base_prompt"]:
            ...
            ph   = st.session_state.get("ad_phase", "")
            em   = st.session_state.get("emotion_emoji", "")
            ...
            if hg and hl:
                ...
                emotion_text = f" · {em}" if em else ""
                phase_text   = f" · Phase: <b>{ph}</b>" if ph else ""
                st.markdown(
                    f"<span style='background:{bc}15;color:{bc};border-radius:6px;padding:4px 10px;font-size:12px'>"
                    f"✅ {gw} · Hair: <b>{hl}</b> · Lighting: <b>{ll}</b>{pose_text}{phase_text}{emotion_text}</span>",
                    unsafe_allow_html=True
                )
            ...
            show_prompt = st.checkbox("Show and edit full prompt", value=False, key="show_prompt_checkbox")
            ...
            if show_prompt:
                edited_prompt = st.text_area(
                    ...
                    help="Edit any part of the prompt. Changes will apply to image generation."
                )

                col_update, col_reset = st.columns(2)
                with col_update:
                    if st.button("📝 Update Prompt", key="update_prompt_btn"):
                        st.session_state["base_prompt"] = edited_prompt
                        st.success("Prompt updated! Now click 'Generate Both' to use edited prompt.")
                        st.rerun()
                with col_reset:
                    if st.button("🔄 Reset to Original", key="reset_prompt_btn"):
                        current_topic = topic if topic else st.session_state.get("last_topic", "")
                        base_style  = st.session_state["ad_data"].get("visual_style", "")
                        base_layout = st.session_state["ad_data"].get("layout", LAYOUTS[0]["groq"])
                        build_and_store(base_style, edited_title, edited_subtext, edited_cta, font_prompt, base_layout)
                        st.success("Prompt reset to original!")
                        st.rerun()

                with st.expander("📋 Current Prompt Preview"):
                    st.code(st.session_state["base_prompt"], language="text")
...
with right_col:
    ...
    CHAT_SYSTEM = (
        ...
        "EMOTION SYSTEM: Problem ads → raw distress. Solution ads → cautious hope. Results ads → genuine confidence. "
        "GENDER: LLM detects gender from medical condition — PCOS/pregnancy → female, prostate → male, others → 50/50. "
